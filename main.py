...
total_year_count = int(input('На сколько лет выдан? '))
proc = int(input('Сколько процентов годовых? ')) # 6
remains = summ
years = total_year_count
first_period = 3
for year in range(1, first_period + 1):
  remains = annuity(remains, proc/100, years, year)
  years -= 1
print('\nОстаток долга:', remains)
print()
print('='*30)
year_count = int(input('\nНа сколько лет продляется договор? '))
proc = int(input('Увеличение ставки до: '))
second_period = total_year_count - first_period + year_count
for year in range(1, second_period + 1):
  remains = annuity(remains, proc/100, second_period, year)
  second_period -= 1
